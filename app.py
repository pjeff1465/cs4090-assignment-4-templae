...
def run_tests(test_path):
    with st.spinner(f"Running {test_path} with coverage..."):
        process = subprocess.Popen(
            ["pytest", test_path, "--cov=tasks", "--cov-report=term", "--cov-report=html:coverage_html"],
        ...
        with st.expander(f"Test Results for {test_path}", expanded=True):
            ...
            if os.path.exists("coverage_html/index.html"):
                ...
                if st.button(f"Open Coverage Report for {test_path}"):
                    try:
...
def main():
    ...
    if st.sidebar.button("Run Advanced Tests"):
        run_tests("tests/test_advanced.py")
    if st.sidebar.button("Run Property Tests"):
        run_tests("tests/test_property.py")
    if st.sidebar.button("Run TDD Tests"):
        run_tests("tests/test_tdd.py")
    if st.sidebar.button("Run Basic Tests"):
        run_tests("tests/test_basic.py")
    if st.sidebar.button("Run All Unit Tests"):
        run_tests("tests/")
